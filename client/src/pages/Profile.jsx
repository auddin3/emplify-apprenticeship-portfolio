...
const Profile = () => {
  ...
  const fName = nameArray.slice(0, -1).join(' ')
  const lName = nameArray.length > 0 ? nameArray[nameArray.length - 1] : ''
  ...
  const handleSave = async () => {
    ...
    const formattedData = {
      name: capitalize(fName) + ' ' + capitalize(lName.trim()),
      ...userDataWithoutName,
    }
    ...
    <div className='bg-gray-paleGray flex flex-row'>
      ...
      {loading
        ...
        : <Box borderWidth='1px' borderRadius='md' bg="white" margin={5} width="full">
          ...
          <Tabs onChange={index => setTabIndex(index)} className='mx-auto' variant='unstyled' size='md' defaultIndex={tabIndex}>
          ...
          <div className='w-full flex flex-row items-center'>
            <Button
              ...
              my={2}
              isDisabled={!dirty}
              className="w-5/12 2xl:w-1/3 rounded-md mx-auto disabled:cursor-not-allowed"
              _active={{
                bg: '#00338D',
                color: 'white',
              }}
              _disabled={{ bg: 'rgba(0,51,141,0.2)' }}
              onClick={handleSave}
            >
                Save
            </Button>
